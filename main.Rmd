...
tx2gene[1:10,]
if(!file.exists("../../Documents/transcript_mapping.csv")){
...
if(!file.exists("../../Documents/phyb_dependent_diff.txt")){
    write.table(sapply(all_diffs, paste0, " 1"), "../../Documents/phyb_dependent_diff.txt", sep = "\n", row.names = F, quote = F)
}
if(!file.exists("../../Documents/phyb_dependent_diff_KEGG.txt")){
...
if(!file.exists("../../Documents/PCAplot.png")){
...
pF<-arrangeGrob(p1,p2, nrow=1, padding = unit(0, "line"))
if(!file.exists("../../Documents/PCAplotDiffs.png")){
...
if(!file.exists("../../Documents/loadedNonDiff.png")){
...
if(!file.exists("../../Documents/loadedDiff.png")){
...
lapply(seq(1:6), function(x){
    name_file<-paste0("../../Documents/", "loadings", colnames(loadInfo[[x]])[2], nam[x], ".png")
    if(!file.exists(name_file)){
        ggsave(filename = name_file, m[[x]], width = 14, height = 8, units = "in")
    }
})
```
...
lapply(seq(1:6), function(x){
    name_file<-paste0("../../Documents/", "GO", nam[x], ".png")
    if(!file.exists(name_file)){
        ggsave(filename = name_file, m[[x]], width = 12, height = 10, units = "in")
    }
})
```
...
```{r}
library(ggplot2)
plotFC.ABS<-function(ebayes.fit, coef, fc.tresh, pval){
    fc<-abs(ebayes.fit$coefficients[,coef])
    expr<-ebayes.fit$Amean
    p.val<-ebayes.fit$p.value
    df.fc.expr<-data.frame(fc = fc, expr = expr, p = p.val)
    df.signif<-df.fc.expr[which(df.fc.expr[,1]>fc.tresh & df.fc.expr[,3]<pval),]
    p<-ggplot(df.fc.expr, aes(expr, fc)) +
        geom_point() +
        geom_point(data = df.signif, aes(y = fc), colour = 'red', size = 2) +
        theme(plot.title = element_text(hjust = 0.5, size = 10)) +
        labs(title=paste("For", colnames(ebayes.fit$coefficients)[coef], "the correlation between\nlow expression and high fold change is\n", round(cor(df.fc.expr$expr, df.fc.expr$fc), digits = 2), "with p.value =", round(cor.test(df.fc.expr$expr, df.fc.expr$fc)[3]$p.value, digits = 2)))
    print(p)
    return(p)
}

plotlist<-lapply(seq(1, ncol(fit.cont.base$coefficients)), function(x){
    plotFC.ABS(fit.cont.base, x, 0, 0.05)
})
```
```{r}
grid.arrange(grobs = plotlist, ncol = 2)
ggsave(file = "../../Documents/fc_abs.png", arrangeGrob(grobs = plotlist, ncol = 2), width = 300, height = 400, units = "mm")
```
```{r}
baseline.change<-lapply(seq(1, ncol(fit.matrix), by = 2), function(x){
    new.mat<-fit.matrix[,c(3,1,4,2,7,5,8,6,11,9,12,10,15,13,16,14)]
    colnames(new.mat)<-rep(c("baseline", "light.change"), 8)
    return(new.mat[,c(x, x+1)])
})

names(baseline.change)<-c("MES.1H", "MES.6H", "DEL.1H", "DEL.6H", "EPI.1H", "EPI.6H", "WT.1H", "WT.6H")
str(baseline.change)
```

```{r}
baseline.plots<-lapply(seq(1, length(baseline.change)), function(x){
    p<-ggplot(baseline.change[[x]], aes(baseline, light.change)) +
        geom_point() +
        geom_abline(intercept = 0, slope = 1, colour = "red") +
        theme(plot.title = element_text(hjust = 0.5, size = 10)) +
        labs(title = paste("Baseline expression versus expression after light change for", names(baseline.change)[x]))
    print(p)
    return(p)
})
```
```{r}
grid.arrange(grobs = baseline.plots, ncol = 2)
ggsave(file = "../../Documents/baseline_change.png", arrangeGrob(grobs = baseline.plots, ncol = 2), width = 300, height = 600, units = "mm")
```
...
if(!file.exists("../../Documents/PCA_baseline.png")){
...
png(filename = "../../Documents/all_base_diff.png", height = 2600, width = 1200, units = "px", res = 200)
grid.draw(gb_fht)
dev.off()
...
```{r}
mes.del.pattern<-function(frame){
    new<-apply(frame,1, function(x){
        if(sign(x[2]) != sign(x[3])){
            ab10<-abs(x[2]-x[3])/10
            
            if(abs(x[2])>ab10 & abs(x[3])>2*ab10 & abs(x[1])<ab10){
                return(x)
            }
        }
    })
    
    newer<-unlist(new[lengths(new) != 0])
    
    name.newer<<-lapply(c(1,2), function(y){
        splt<-unlist(strsplit(names(newer), "\\."))
        return(unique(splt[seq(y, length(splt), 2)]))
    })
    
    newest<-data.frame(matrix(newer, ncol = 3, byrow = T, dimnames = name.newer))
    return(newest)
}

pattern.genes1<-mes.del.pattern(base.diff[,1:3])
pattern.genes6<-mes.del.pattern(base.diff[,4:6])

pattern.genes<-base.diff[unique(c(rownames(pattern.genes1), rownames(pattern.genes6))),]

pattern.genes[1:10,]
```
```{r, fig.height=8, fig.width=12}
mdht1<- Heatmap(
  as.matrix(pattern.genes1), 
  name="pattern.genes1", 
  clustering_distance_rows = "pearson",
  column_title = "Differentially expressed genes \n at steady state low light at 1H, showing mes>WT|epi>del pattern", 
  column_title_side = "top", 
  column_names_rot = 45, 
  width = unit(9, "cm"), 
  height = unit(16, "cm"), 
...
mdht6<- Heatmap(
  as.matrix(pattern.genes6), 
  name="pattern.genes6", 
  clustering_distance_rows = "pearson",
  column_title = "Differentially expressed genes \n at steady state low light at 6H, showing mes>WT|epi>del pattern", 
  column_title_side = "top", 
  column_names_rot = 45, 
  width = unit(9, "cm"), 
  height = unit(16, "cm"), 
...
gb_mdht1 = grid.grabExpr(draw(mdht1))
gb_mdht6 = grid.grabExpr(draw(mdht6))
...
```
```{r}
pattern.info<-tair_bindings[rownames(pattern.genes),]


if(!file.exists("../../Documents/mes_del_pattern_info.txt")){
    write.table(pattern.info, file = "../../Documents/mes_del_pattern_info.txt", quote = F, sep = "\t")
}
    
```

Genes that show pattern and diff in both 1 and 6 hours

```{r}
int.pattern<-intersect(rownames(pattern.genes1), rownames(pattern.genes6))
int.pattern<-base.diff[int.pattern,]

int.patt.info<-cbind(int.pattern, tair_bindings[rownames(int.pattern),])

if(!file.exists("../../Documents/intersect_pattern.txt")){
    write.table(int.patt.info, file = "../../Documents/intersect_pattern.txt", quote = F, sep = "\t")
}
```
...
```{r}
chrom<-read.table("/mnt/uosfstore/sudlab1/General/projects/magda_RNAseq_MAIN/R/Documents/TE_coords.txt", header=T)
chrom$orientation_is_5prime<-gsub("true", "+", chrom$orientation_is_5prime)
chrom$orientation_is_5prime<-gsub("false", "-", chrom$orientation_is_5prime)
chrom$chromosome<-apply(chrom, 1, function(x){
    y<-as.numeric(strsplit(unfactor(x[1]), "")[[1]][3])
    return(y)
})
chrom.GR<-makeGRangesFromDataFrame(chrom, seqnames.field = "chromosome", start.field = "Transposon_min_Start", end.field = "Transposon_max_End", strand.field = "orientation_is_5prime", keep.extra.columns = T)
names(chrom.GR)<-chrom$Transposon_Name
chrom.GR[1:5]
```

```{r}
te.genes<-GenomicFeatures::genes(arb)
te.genes[1:5]
```

```{r}
near.result<-GenomicRanges::nearest(te.genes[rownames(int.pattern)[1]], chrom.GR)
near.dist<-GenomicRanges::distanceToNearest(te.genes[rownames(int.pattern)[1]], chrom.GR)
te.genes[rownames(int.pattern)[1]]
chrom.GR[near.result]
near.dist
```
```{r}
closest.te<-lapply(rownames(int.pattern), function(x){
    query<-x
    n.result<-GenomicRanges::nearest(te.genes[x], chrom.GR)
    n.te<-names(chrom.GR[n.result])
    d<-GenomicRanges::distanceToNearest(te.genes[x], chrom.GR)
    d.te<-elementMetadata(d)[1,1]
    return(c(query, n.te,d.te))
})

closest.te<-data.frame(matrix(unlist(closest.te), ncol=3, byrow = T))

sample.mean<-mean(unfactor(closest.te[,3]))
sample.mean
```

```{r}
#av.te.proxy<-sapply(seq(1,32548), function(x){
#    d<-GenomicRanges::distanceToNearest(te.genes[x], chrom.GR)
 #   d.te<-elementMetadata(d)[1,1]
  #  print(paste("done", x))
   # return(d.te)
...
```{r}
full.mean<-mean(av.te.proxy)
full.mean
```
```{r}
plot(density(unfactor(closest.te[,3])));plot(density(av.te.proxy))
```
```{r}
qqnorm(av.te.proxy);qqline(av.te.proxy, col = 2)
qqnorm(unfactor(closest.te[,3]));qqline(unfactor(closest.te[,3]), col = 2)
```


```{r}
t.test(unfactor(closest.te[-10,3]), av.te.proxy, alternative = "two.sided", var.equal = FALSE)
```
```{r}
wilcox.test(unfactor(closest.te[,3]), av.te.proxy)
```
```{r}
ks.test(unfactor(closest.te[,3]), av.te.proxy, alternative = "two.sided")
```
```{r}
ks.test(av.te.proxy, unfactor(closest.te[,3]), alternative = "less")
```


DIGITISED expression
...
```
```{r}
if(!file.exists("../../Documents/auxin.bed")){
...
if(!file.exists("../../Documents/diff_info.txt")){
...
if(!file.exists("../../Documents/TF_analysis/LHY.csv")){
