...
```
```{r}
#write.table(rownames(defense.base.diff), file = "../../Documents/defense_names.txt", sep = "\n", quote = FALSE, row.names = FALSE, col.names = FALSE)
```

```{r}
defense.names<-read.table("../../Documents/defense_names.txt", header = TRUE, sep = "\t", fill = TRUE, stringsAsFactors = FALSE)
...
rownames(named.defense.base.diff)<-sapply(rownames(named.defense.base.diff), function(x){
  if(defense.names[x,4]==""){
    return(x)
  } else{
    return(defense.names[x,4])
  }
})
```
